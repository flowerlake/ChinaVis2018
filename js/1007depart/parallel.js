...
var myChartparallel = echarts.init(document.getElementById("main"),'dark');

$.get("data/groups/smallg/gfirst1007.json", function(legend_data) {
	$.get("data/groups/smallg/ggroup1007.json", function(groups) {
		$.get("data/parallel/tcp1007.json", function(data) {
			var schema = [{
					name: 'client_ip',
					index: 0,
					text: 'client_ip'
...
//				legroup[v[i]] = k;
			...
			for(i = 0; i < legend_data.length; i++) {
				option.series.push({
					...
					type: 'parallel',
					lineStyle: lineStyle,
					data: data[legend_data[i].replace('小组','')]
