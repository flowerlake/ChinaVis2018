var ConceptMap = function(chartElementId, infoElementId, dataJson) {
	...
	function b(X) {
		...
		Y.append("text").attr("stroke", "#F8F8F8").attr("stroke-width", 4).attr("class", "label-stroke");
		Y.append("text").attr("font-size", 17).attr("class", "label");
...
//				return "ITEM " + ae.ditem
			...
			if (ab && ab.type === "theme") {
				Y.append("text").attr("fill", "#aaa").attr("text-anchor", "middle").attr("y", (o + t) * -1).text("server_ip")
			} else {
				if (ab && ab.type === "perspective") {
					...
					Y.append("text").attr("fill", "#aaa").attr("text-anchor", "middle").attr("y", (o + t) * -1).text("email subject");
					ad.exit().remove()
				}
			}
		}
		...
		X.enter().append("text").attr("text-anchor", "start").attr("x", a / -2 + t).attr("y", c / 2 - t).attr("class", "all-ditems").on("click", O);
		X.exit().remove()
	}
	...
	function m(Y) {
		...
		}).attr("stroke", "#0da4d3").attr("stroke-width", 1.5).transition().duration(w).ease(F).attr("d", function(Z) {
			return v([
				[Z.x1, Z.y1],
				[Z.xx, Z.yy],
				[Z.x2, Z.y2]
...
//			return (X === L.node || X.isGroup) ? "#fff" : l(X, "#0da4d3", N, "#999")
//		})
	}
...
{
	d3.json("data/map/map1059.json", function(dataJson) {
		var plot = new ConceptMap("concept", "graph-info", dataJson);
	});
}