	...
	function chart(selection) {
		selection.each(function drawGraph(dataset) {
			...
			var svg = d3.select(this).append('svg')
				.attr('width', width + margin.left + margin.right-310)
				.attr('height', height + margin.top + margin.bottom + 129)
				.append('g')
			...
			if (drawTitle) {
				svg.select('#g_title')
					.append('text')
					.attr('x', paddingLeft+310)
					.attr('y', paddingTopHeading)
					.text('研发2部的打卡信息、tcp连接以及login信息对比')
					.attr('style',"text-align:center;color:antiquewhite");
//					.attr('class', 'heading');
...
var chart = visavailChart().width(1000);
